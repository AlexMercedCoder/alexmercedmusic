...
const expectedTools = ['music_overview', 'get_song', 'search_songs', 'list_songs', 'list_reimaginings', 'list_albums', 'where_to_listen', 'navigate_catalog'];
...
console.log(`Validated ${catalog.songs.length} song pages, ${catalog.albums.length} album pages, ${catalog.songs.reduce((count, song) => count + song.links.length, 0)} listening links, sitemap coverage, structured data, and ${registered.length} WebMCP tools.`);
